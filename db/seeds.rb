...
tools.each do |tool|
  Tool.create(name: tool[:name], price: rand(0.7..2.5).round(2), user: latif, image_url: tool[:image_url], description: tool[:description], city: tool[:city])
end

5.times do
  tool = tools.sample
  Tool.create(name: tool[:name], price: rand(0.7..2.5).round(2), user: pat, image_url: tool[:image_url], description: tool[:description], city: tool[:city])
end
